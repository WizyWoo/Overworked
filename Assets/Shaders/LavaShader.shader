    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(v2f IN) : SV_Target
            {
                ...
                half4 color = SAMPLE_TEXTURE2D(_Texture, sampler_Texture, float2((IN.uv.x * _TexSize) + (IN.wPos.x + IN.wPos.y / 2) + (_Time[1] * _FlowDirX), 
                (IN.uv.y * _TexSize) + (IN.wPos.z + IN.wPos.y / 2) + (_Time[1] * _FlowDirY)));
                return color;
