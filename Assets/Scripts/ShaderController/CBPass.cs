...
internal class CBPass : ScriptableRenderPass
{
    ...
    public CBPass(Material material)
    {
        m_Material = material;
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        using (new ProfilingScope(cmd, m_ProfilingSampler))
        {
            ...
            cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.Translate(camera.transform.position + camera.transform.forward), m_Material);
        }
        context.ExecuteCommandBuffer(cmd);
        cmd.Clear();
        Debug.Log("I did a run");
