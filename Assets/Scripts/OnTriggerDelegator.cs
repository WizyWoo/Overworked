...
public class OnTriggerDelegator : MonoBehaviour
{
    ...
    public UnityEvent<OnTriggerDelegation3D> Enter;

    [Tooltip("Which function should be called when trigger was exited.")]
    public UnityEvent<OnTriggerDelegation3D> Exit;


    void OnTriggerEnter(Collider other) => Enter.Invoke(new OnTriggerDelegation3D(caller, other));
    void OnTriggerExit(Collider other) => Exit.Invoke(new OnTriggerDelegation3D(caller, other));
}


public struct OnTriggerDelegation3D
{
    ...
    public OnTriggerDelegation3D(Collider caller, Collider other)
    {
        Caller = caller;
        Other = other;
    }
